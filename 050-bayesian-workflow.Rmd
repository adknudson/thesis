...
model {
  ...
  beta ~ lognormal(2.5, 1);  // Prior for slope
...
```{r}
av_iter1_sim <- stan_model(file = "models/av_iter1_sim.stan", 
                           model_name = "av_iter1_sim")

sim_fit <- sampling(av_iter1_sim, data = sim_dat, 
...
```{r}
av_iter1_sim_fit <- stan_model(file = "models/av_iter1_sim_fit.stan", 
                               model_name = "av_iter1_sim_fit")

sim_fit <- sampling(av_iter1_sim_fit, data = sim_dat, refresh = 0)
...
av_iter1_obs_fit <- stan_model("models/av_iter1_obs_fit.stan",
                               model_name = "av_iter1_obs_fit")

m1_fit <- sampling(av_iter1_obs_fit, data = obs_dat, refresh = 0)
```
...
idx <- sample(1:nrow(m1_post$y_post_pred), 1)

m1_pred <- cbind(m1_post_k_pred, 
                 post_mean = colMeans(m1_post$y_post_pred),
                 post_rand = m1_post$y_post_pred[idx,]) %>%
