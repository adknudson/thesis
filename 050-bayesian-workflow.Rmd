...
```{stan ch050-Pointless Doorstop, output.var="av_iter1_sim"}
data {
  int<lower=0> N;
  int n[N];
  vector[N] x;
...
```{stan ch050-Maroon Vulture, output.var="av_iter1_sim_fit"}
data {
  int N;
  int n[N];
  int k[N];
...
```{stan ch050-Golden Locomotive, output.var="av_iter1_obs_fit"}
data {
  int N;
  int n[N];
  int k[N];
