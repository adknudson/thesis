...
generated quantities {
  ...
  real alpha = normal_rng(0, 0.05);
  real alpha_T = normal_rng(0, 0.05);
  real<lower=0> sigma_aG = half_cauchy_rng(0.1);
  real<lower=0> sigma_aTG = half_cauchy_rng(0.1);
  real alpha_G[N_G] = normal_rng(rep_vector(0, N_G), sigma_aG);
  real alpha_TG[N_G] = normal_rng(rep_vector(0, N_G), sigma_aTG);
  
  real beta = normal_rng(0, 0.05);
  real beta_T = normal_rng(0, 1);
  real<lower=0> sigma_bG = half_cauchy_rng(1.0);
  real<lower=0> sigma_bTG = half_cauchy_rng(1.0);
  ...
  for (i in 1:N_G) {
    ...
    real gamma_post = exp(beta + beta_G[i] + (beta_T + beta_TG[i]) * trt[i]);
    real delta_pre = alpha + alpha_G[i];
    real delta_post = alpha + alpha_G[i] + (alpha_T + alpha_TG[i]) * trt[i];
...
```{r}
sim_fit <- sampling(sim_iter2, data = sim_dat, 
                    algorithm="Fixed_param", refresh = 1000)
sim_prior <- extract(sim_fit)
```
