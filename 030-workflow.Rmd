...
```{r ch032-Empty Sledgehammer, echo=TRUE}
str(p032$k_pred)
```
...
model {
  ...
  for (i in 1:N) {
    real mu_b = b + bG[G[i]] + bT[trt[i]] + bGT[G[i], trt[i]];
    real mu_a = a + aG[G[i]] + aT[trt[i]] + aGT[G[i], trt[i]];
    theta[i] = exp(mu_b) * (x[i] - mu_a);
  }
...
if (file.exists("models/m033.rds")) {
  ...
  m033 <- sampling(m033_stan, data = obs_dat, seed = 143,
                   iter = 5000, warmup = 2500, pars = keep_pars,
                   control = list(adapt_delta = 0.95),
                   chains = 4, cores = 4, refresh = 0)
...
if (file.exists("models/m033vis.rds")) {
  ...
  m033vis <- sampling(m033_stan, data = obs_dat, seed = 143,
                      iter = 5000, warmup = 2500, pars = keep_pars,
                      control = list(adapt_delta = 0.95),
                      chains = 4, cores = 4, refresh = 0)
...
if (!file.exists("figures/ch033-Iron-Intensive.png")) {
  ...
  for (i in 1:3) {
    p <- data.frame(x = c(-0.1, 0.1), y = c(0, 1)) %>% 
      ...
      theme(axis.title.y = element_blank()) +
      labs(x = "SOA (seconds)")
    alpha_pre <- with(p033vis, a + aG[,i] + aT[,1] + aGT[,i,1])
    alpha_pos <- with(p033vis, a + aG[,i] + aT[,2] + aGT[,i,2])
    beta_pre <- exp(with(p033vis, b + bG[,i] + bT[,1] + bGT[,i,1]))
    beta_pos <- exp(with(p033vis, b + bG[,i] + bT[,2] + bGT[,i,2]))
    for (j in idx) {
      p <- p +
...
if (file.exists("models/m034.rds")) {
  ...
  m034 <- sampling(m034_stan, data = obs_dat, seed = 626, init = init,
                   iter = 7000, warmup = 2000, pars = keep_pars,
                   control = list(adapt_delta = 0.95),
...
if (!file.exists("figures/ch034-Screaming-Proton.png")) {
  ...
  for (i in 1:3) {
    p <- data.frame(x = c(-0.1, 0.1), y = c(0, 1)) %>% 
      ...
      theme(axis.title.y = element_blank()) +
      labs(x = "SOA (seconds)")
    alpha_pre <- with(p034, a + aG[,i] + aT[,1] + aGT[,i,1])
    alpha_pos <- with(p034, a + aG[,i] + aT[,2] + aGT[,i,2])
    beta_pre <- exp(with(p034, b + bG[,i] + bT[,1] + bGT[,i,1]))
    beta_pos <- exp(with(p034, b + bG[,i] + bT[,2] + bGT[,i,2]))
