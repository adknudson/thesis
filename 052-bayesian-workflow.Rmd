...
```{stan ch052-Forsaken Confidential Ostrich, output.var="av_iter3_obs_fit"}
data {
  int N;
  int N_G;
  int N_T;
...
```{r ch052-Dreaded Elastic Metaphor}
...
```{stan ch052-Eager Modern Parachute, output.var="av_iter4_obs_fit"}
functions {
...
```{r ch052-Furious Ninth Xylophone}
m5_4_1_pred %>%
  ggplot(aes(x, p)) +
  scale_x_continuous(breaks = seq(-0.5, 0.5, 0.25)) +
  scale_y_continuous(breaks = c(0, 0.5, 1)) +
