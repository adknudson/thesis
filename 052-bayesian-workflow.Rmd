...
```{stan output.var="av_iter3_obs_fit"}
data {
  int N;
  int N_G;
  int N_T;
...
m5_3_1 <- sampling(av_iter3_obs_fit, data = obs_dat, 
                   chains = 10, cores = 10, iter = 3000, warmup = 2000,
...
The number of effective samples and the Rhat indicate that there is no problem with the posterior samples.
...
Okay what gives? The differences within and between age groups is not a separated as it was in the previous iteration. This is due to the fact that the previous model averaged over the variation at the subject level.

## Iteration 4

```{stan output.var="av_iter4_obs_fit"}
functions {
...
init5_4_1 <- replicate(n_chains, list(
  a = 0, 
  aG = rep(0, obs_dat$N_G), 
  aT = rep(0, obs_dat$N_T), 
  aS = rep(0, obs_dat$N_S),
  b = 3.0,
  bG = rep(0, obs_dat$N_G),
  bT = rep(0, obs_dat$N_T),
  bS = rep(0, obs_dat$N_S),
  lG = rep(0.02, obs_dat$N_G)),
simplify = FALSE)

m5_4_1 <- sampling(av_iter4_obs_fit, data = obs_dat, 
                   chains = 10, cores = 10, iter = 7000, warmup = 5000,
                   refresh = 100, control = list(adapt_delta = 0.95),
                   pars = keep_pars, init = init5_4_1)
```
