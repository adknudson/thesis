```{r ch070-setup, include=FALSE, cache=FALSE}
library(tidyverse)
source("R/ch070.R")
```
...
```{r ch070-Lama Everyday, out.width="30%"}
...
```{r ch070-Third Needless Antique, out.width="35%"}
...
```{r ch070-Lonesome Helium, echo=TRUE}
table(feat_typ[,4])
```

```{r ch070-Brave Gamma, echo=TRUE}
table(feat_atyp[,4])
```
...
```{r ch070-Remote Monkey, echo=TRUE}
glimpse(features)
```
...
```{r ch070-Rubber Modern, echo=TRUE, eval=FALSE}
