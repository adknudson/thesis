...
There are common situations where MLE fails such as complete separation in the data. This is when the positive class can be separated from the negative class by a set of linear predictors (shown in figure \@ref(fig:ch020-comp-sep)).


```{r ch020-comp-sep, fig.cap="Example of complete separation in the data. All of the \`0\`-responses can be separated from the \`1\`-responses by some value of \`x\` between \`-1\` and \`0\`."}
y <- c(0, 0, 1, 1, 1)
x <- c(-2, -1, 0, 1, 2)
...
```{r ch020-Rubber Blue, echo=TRUE, warning=TRUE, message=FALSE}
...
However, this is not the true maximum likelihood estimate. Table \@ref(tab:ch020-Quality-Surreal-Street) shows that the log-likelihood is still increasing as the scale decreases to zero. There is a very large change between $-2.05\times 10^{-10}$ and $-3.86\times10^{-22}$ on a relative scale, but computers cannot tell the difference on an absolute scale. The precision of most modern computers is around $10^{-15}$, and anything smaller is numerically zero.


```{r ch020-Quality-Surreal-Street}
tibble(
